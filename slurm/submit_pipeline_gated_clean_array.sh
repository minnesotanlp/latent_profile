...
CONFIG_PATH="${CONFIG_PATH:-$PROJECT_DIR/configs/experiments/core_clean.yaml}"
MODELS_PATH="${MODELS_PATH:-$PROJECT_DIR/configs/models.yaml}"
RUN_ROOT="${RUN_ROOT:-/lustre/fs0/scratch/jmooney/latent_profile}"
MANIFEST_DIR="$RUN_ROOT/manifests"
MANIFEST_PATH="${MANIFEST_PATH:-$MANIFEST_DIR/gated_clean_model_ids.txt}"
...
if [[ "$COUNT" -eq 0 ]]; then
  echo "Gated clean manifest is empty: $MANIFEST_PATH" >&2
  exit 2
fi

echo "Submitting gated clean array for $COUNT models using manifest $MANIFEST_PATH"
