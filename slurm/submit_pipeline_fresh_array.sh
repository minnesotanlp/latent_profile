...
CONFIG_PATH="${CONFIG_PATH:-$PROJECT_DIR/configs/experiments/core_fresh.yaml}"
RUN_ROOT="${RUN_ROOT:-/lustre/fs0/scratch/jmooney/latent_profile}"
MANIFEST_DIR="$RUN_ROOT/manifests"
MANIFEST_PATH="${MANIFEST_PATH:-$MANIFEST_DIR/fresh_model_ids.txt}"
...
if [[ "$COUNT" -eq 0 ]]; then
  echo "Fresh manifest is empty: $MANIFEST_PATH" >&2
  exit 2
fi

echo "Submitting fresh full array for $COUNT models using manifest $MANIFEST_PATH"
